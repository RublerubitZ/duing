...
⚠️ **timestamptz 컬럼 + seoul 벽시계 저장(면접·회비 일부)은 DB에 기록된 절대시각이 실제보다 +9h 왜곡**되어 있었다(JDBC 세션 존=UTC로 캐스팅되기 때문). 앱 왕복은 벽시계가 복원되어 정상이지만, SQL 직접 조회·CSV·BI에서는 틀린 값이었다. 2단계 백필의 `- interval '9 hours'` 보정 대상이 바로 이 컬럼들이고, 아래 대응표에서 ⚠️ 로 표시했었다 — **해당 4컬럼 전부 백필 + Instant 전환 완료(2026-08-22)** 로 왜곡 적립이 끝났다: `payment.paid_at`·`payment.voided_at`·`bank_transaction.transaction_at` 은 V112(2026-08-21), `interview_round.assignment_completed_at` 은 V113. 넷 다 저장 코드가 절대시각을 쓰고, 운영의 기존 행은 각 백필이 −9h 보정한다. 대응표에 남은 ⚠️ 행은 이제 없다.
...
| PublicActivityResponse.Item.occurredAt (INTERVIEW_RESULT 피드만) | interview_round.assignment_completed_at (timestamptz) | **Instant** | GeneralInterviewAssignmentService#confirmRound 가 `Instant.now(clock)` 을 InterviewRound#confirm 에 넘겨 절대시각으로 저장 — 읽기도 PublicActivityQueryRepository#findRecentInterviewResult 가 TimeMapper 없이 직비교·직반환(2026-08-21 에 넣었던 seoul 해석 축은 제거). 위 5종(system)과 저장 regime 만 갈리고 응답 절대시각은 정합. V113 백필 완료(2026-08-22) |
...
1. 위 regime 대응표가 백필 명세다. 컬럼별로:
   ...
   - seoul(이미 timestamptz, +9h 왜곡) → `- interval '9 hours'` 보정. 대응표에서 ⚠️ 였던 4컬럼이 전부 여기 해당하며 **모두 보정 완료**다 — `payment.paid_at`·`payment.voided_at`·`bank_transaction.transaction_at`(**V112 로 보정 완료, 2026-08-21**)·`interview_round.assignment_completed_at`(**V113 으로 보정 완료, 2026-08-22**). 넷 다 저장 코드도 같은 릴리스에서 Instant 전환했다
...
4. **환경 분기는 Flyway placeholder 로 가드한다.** +9h 왜곡은 JVM=UTC 인 운영에만 쌓였고 dev/local(JVM=KST)의 같은 컬럼은 이미 정합이라, 모든 환경에 백필을 돌리면 개발 DB 가 오히려 −9h 로 오염된다. 그래서 V112·V113 은 `WHERE ${apply_tz_backfill}` 형태로 쓰고, 공통 `application.yml`·테스트 `application.yml` 은 `"false"`(no-op), `application-prod.yml` 만 `"true"` 로 둔다(레포 첫 placeholder 사용). Flyway 체크섬은 치환 전 원문 기준이라 환경 간 불일치가 생기지 않고, placeholder 를 정의하지 않은 환경은 부팅 단계에서 즉시 실패해 조용한 누락이 없다.
