...
| PublicActivityResponse.Item.occurredAt (공개 활동 피드) | recruitment·notice·interview_round·club_event·fee_policy.created_at (5종) | system | PublicActivityQueryRepository#toInstant 가 6개 피드 전부에 systemWallClockToInstant 적용 — 5종은 BaseEntity 감사라 정합 |
| ⚠️ PublicActivityResponse.Item.occurredAt (INTERVIEW_RESULT 피드만) | interview_round.assignment_completed_at | **seoul** | GeneralInterviewAssignmentService#confirmRound 가 now(clock) 인데 읽기는 위 공용 systemWallClockToInstant — **regime 불일치**(아래 알려진 이슈 3) |
...
3. **공개 활동 피드 INTERVIEW_RESULT 의 regime 불일치** — `interview_round.assignment_completed_at` 은 `GeneralInterviewAssignmentService#confirmRound` 가 `now(clock)`(KST 벽시계)로 쓰는데, `PublicActivityQueryRepository#toInstant` 는 6개 피드 전부에 `systemWallClockToInstant` 를 적용한다. 로컬(JVM=KST)에서는 우연히 맞지만 **prod(JVM=UTC)에서는 이 피드 항목만 발생 시각이 9시간 늦게 표기**되고, 조회 윈도우(`since`) 비교도 같은 폭만큼 어긋난다. 다른 5종은 BaseEntity 감사라 정합. 후속 PR 대상(피드 한정, 데이터 무손상).
