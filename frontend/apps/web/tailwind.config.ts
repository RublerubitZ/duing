...
const config: Config = {
  ...
  theme: {
    extend: {
      ...
      maxWidth: {
        // 레이아웃 폭 상한. 좌우 여백(px-4 sm:px-6 md:px-10)은 반드시 이 상한을 쓰는 요소 "안쪽" 에 둔다
        // — 바깥 래퍼에 두면 콘텐츠가 1280 까지 벌어져 같은 화면의 상단바·티커(콘텐츠 폭 1200 규약,
        // 시안 ×0.815 환산의 기준)와 어긋난다. 두 방식은 vw≤1280 에서만 결과가 같고 1281~1359 에서
        // 0→40px 로 벌어진 뒤 1360 부터 40px 로 고정된다 — 좁은 창에서 눈으로 확인하고 넘기지 말 것.
        layout: '1280px',
      },
      letterSpacing: {
        tightx: '-0.02em',
