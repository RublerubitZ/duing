...
// 부원 이름·학번 해석용. buildBill 의 userId(42, 99) 와 매칭되는 부원을 돌려준다.
const members = [
  {
    memberId: 1,
    userId: 42,
...
describe('BillList', () => {
  ...
  it('행에 부원 이름·학번·회차·금액·마감일과 상태 뱃지를 표시한다', () => {
    ...
    // userId 42 → 김민지(20210001) 로 해석되어 `부원 #id` 대신 이름·학번이 보인다.
    expect(within(row).getByText('김민지')).toBeInTheDocument();
    expect(within(row).getByText('20210001')).toBeInTheDocument();
    expect(within(row).queryByText('부원 #42')).not.toBeInTheDocument();
  ...
  it('부원을 못 찾으면(탈퇴 등) `부원 #id` 로 폴백한다', () => {
    mockUseClubFeeBillsQuery.mockReturnValue({
    ...
    render(<BillList clubId={1} />);
    const row = screen.getByRole('listitem');
    expect(within(row).getByText('부원 #1818')).toBeInTheDocument();
  });
  ...
  it('부원 검색에서 이름으로 찾아 선택하면 userId 필터로 재조회하고, 칩 해제 시 필터가 풀린다', () => {
    mockUseClubFeeBillsQuery.mockReturnValue({ data: buildPage([buildBill()]), isLoading: false });
    render(<BillList clubId={1} />);

    // 이름 일부를 입력하면 일치하는 회원이 드롭다운에 뜬다.
    fireEvent.change(screen.getByRole('textbox', { name: '부원 검색' }), {
      target: { value: '서준' },
    });
    ...
    fireEvent.click(screen.getByRole('button', { name: '부원 필터 해제' }));
