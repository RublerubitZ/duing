...
// 10 phase 전수 매핑 — 신 phase 추가 시 여기서 컴파일 에러로 누락 감지.
describe('getInterviewPhaseGuide', () => {
  it('NOT_APPLICABLE → null', () => {
    expect(getInterviewPhaseGuide('NOT_APPLICABLE')).toBeNull();
  });

  it('DOCUMENT_REVIEW → stepIndex 1, 서류 검토 안내 문구', () => {
    const guide = getInterviewPhaseGuide('DOCUMENT_REVIEW');
    expect(guide).not.toBeNull();
    expect(guide!.stepIndex).toBe(1);
    expect(guide!.title).toBe('서류 검토 중');
    expect(guide!.description).toBeTruthy();
  });

  it('WAITING_ROUND → stepIndex 2, 회차 대기 문구', () => {
  ...
  it('WAITING_NEXT_ROUND → stepIndex 2, 다음 회차 대기 문구', () => {
  ...
  it('AVAILABILITY_REQUESTED → stepIndex 2, 가능 시간 선택 안내', () => {
  ...
  it('AVAILABILITY_CLOSED → stepIndex 2, 제출 마감 안내', () => {
  ...
  it('RESPONDED → stepIndex 2, 응답 완료 안내', () => {
  ...
  it('NO_SLOT_REPORTED → stepIndex 2, 조율 요청됨 안내', () => {
  ...
  it('SCHEDULING → stepIndex 2, 배정 검토 중 안내', () => {
  ...
  it('SCHEDULED → stepIndex 3, 일정 확정 안내', () => {
    const guide = getInterviewPhaseGuide('SCHEDULED');
    expect(guide).not.toBeNull();
    expect(guide!.stepIndex).toBe(3);
  ...
  it('모든 non-null guide 에 title·description 이 non-empty 문자열이다', () => {
    const phases: ApplicantInterviewPhase[] = [
      'DOCUMENT_REVIEW', 'WAITING_ROUND', 'WAITING_NEXT_ROUND',
