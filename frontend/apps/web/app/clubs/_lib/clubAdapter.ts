...
import { isDivision, type Club, type ClubCat, type ClubScope, type ClubStatus } from './clubs';
...
const deriveScope = (division: string | null): ClubScope =>
  isDivision(division) ? '중앙' : '과';
...
export function summaryToClub(summary: ClubSummary): Club {
  ...
  const scope = deriveScope(summary.division);
  const division = isDivision(summary.division) ? summary.division : null;
