import { ClubMemberNoticesPage } from './_pages/ClubMemberNoticesPage';

// 이 서버 셸을 정적(Full Route Cache 대상)으로 잡아 <Link> 뷰포트 프리페치를 CDN HIT 으로 받게 한다
// — 동아리 상세(#915)·공지 상세(#945)와 동일 처방. 회원 페이지 탭(MemberPageHeader)·공개 상세 '소식'
// 탭·마이페이지 둘러보기 링크의 프리페치가 clubId 마다 서버리스 함수를 깨우던 것을 차단한다(Active CPU).
...
// clubId 는 클라이언트가 useParams 로 읽는다. 접근 제어도 layout 의 클라이언트 MemberAccessGuard 가 맡는다).
// generateMetadata 나 서버 데이터 fetch 를 추가하는 순간 이 전제가 깨져 배포 수명 내내 stale 이 서빙된다.
// 그때는 finite revalidate 와 fail-soft 를 반드시 함께 넣을 것.
//
