...
export default function ClubsPage() {
  ...
  const clubListQuery = useClubListQuery(params);

  return (
    <main className="mx-auto max-w-5xl px-6 py-10">
      <header className="mb-8">
        <h1 className="text-3xl font-bold tracking-tight">동아리 탐색</h1>
      ...
      <section>
        {clubListQuery.isLoading && <p className="text-sm text-slate-500">불러오는 중…</p>}
        {clubListQuery.error && (
        ...
        {clubListQuery.data && (
          <>
            ...
            {clubListQuery.data.totalPages > 1 && (
              <Pagination
                page={params.page ?? 0}
                totalPages={clubListQuery.data.totalPages}
