...
export default function FacilityDetailPage({
  ...
  const usingNow =
    ...
    detailResponse.facility.isUsingNow &&
    detailResponse.facility.currentReservation !== null;
    ...
    <section className="bg-cream px-4 sm:px-6 md:px-10 pb-20 pt-8">
      <div className="max-w-layout mx-auto">
          ...
          <>
            <div className="mb-2 flex items-center gap-2">
              ...
              <span
                ...
                style={{ color: usingNow ? '#2E6149' : '#6F7574' }}
              >
                {usingNow ? '현재 사용 중' : '현재 이용 가능'}
              </span>
            </div>
            <h1 className="text-[28px] tracking-tightx md:text-[36px]" style={{ color: '#1F4A36' }}>
              {detailResponse.facility.roomName}
            </h1>
            {detailResponse.facility.location && (
              <p className="mt-1 text-[14px] text-charcoal-2">{detailResponse.facility.location}</p>
            )}
            <div className="mt-5">
              <FacilityUpdateBanner
                lastUpdatedAt={detailResponse.lastUpdatedAt}
                stale={detailResponse.stale}
              />
            </div>
          </>
        )}
        ...
        {/* 월 이동 — 현재월 ±12개월 경계에서 비활성화(§16.2).
            로딩·에러 중에도 유지해 미캐시 월(온디맨드 수집 지연/실패)에서 되돌아올 수 있게 한다. */}
        ...
        <div className="mt-3">
          {detail.isLoading ? (
            ...
            <FacilityTimeline
              key={yearMonth}
              reservations={detailResponse.facility.reservations}
              yearMonth={yearMonth}
            />
          ) : (
            <p className="p-6 text-sm text-coral">시설을 찾을 수 없습니다.</p>
          )}
        </div>
      </div>
    </section>
