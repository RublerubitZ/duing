...
/**
 * 목업 슬롯 스트립 톤 매핑 — 가능=sage-mist · 점유=graysoft+조직명 · 신청=ink · 충돌=coral · 반영=sage.
 * overlaps 는 BE 관리자 상세가 SCHOOL·INTERNAL·OWN 을 싣는다(대기 겹침은 overlappingPendingCount 숫자).
 * OWN(학교가 이 동아리 이름으로 등록한 행)은 신청 구간 안에서 충돌이 아니라 '반영'이다 — 그 외 겹침 항목은
...
function cellVisual(cell: SlotStripCell): { tone: string; label: string } {
  ...
  if (cell.inRequest && cell.overlapSource === 'OWN') return { tone: 'bg-sage text-ink-deep', label: '반영' };
