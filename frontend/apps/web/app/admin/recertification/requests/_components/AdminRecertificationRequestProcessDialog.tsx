...
const TERMINAL_STATUSES: Exclude<RecertificationStatus, 'PENDING'>[] = ['APPROVED', 'REJECTED'];
...
export function AdminRecertificationRequestProcessDialog({
  ...
  const [selectedStatus, setSelectedStatus] = useState<Exclude<RecertificationStatus, 'PENDING'>>('APPROVED');
  ...
  return (
    ...
    >
      ...
      >
        <DialogHeader>
          <DialogTitle>재인증 요청 처리</DialogTitle>
          <DialogDescription>
            요청 #<span className="font-medium text-charcoal-2">{request.id}</span> — {request.club.name}
          </DialogDescription>
        </DialogHeader>
