...
type Props = {
  request: AdminRecertificationRequestDetail;
  isPending: boolean;
  errorMessage: string | null;
  onConfirm: (payload: ProcessRecertificationPayload) => void;
  onCancel: () => void;
};

const ACTION_NOTE_MAX = 1000;
const TERMINAL_STATUSES: Exclude<RecertificationStatus, 'PENDING'>[] = [
  'APPROVED',
  'REJECTED',
];
const STATUS_LABEL_MAP: Record<Exclude<RecertificationStatus, 'PENDING'>, string> = {
  APPROVED: '승인',
  REJECTED: '거절',
};

export function AdminRecertificationRequestProcessDialog({
  request,
  isPending,
  errorMessage,
  onConfirm,
  ...
  const [selectedStatus, setSelectedStatus] =
    useState<Exclude<RecertificationStatus, 'PENDING'>>('APPROVED');
  const [actionNote, setActionNote] = useState('');

  const handleSubmit = () => {
    const payload: ProcessRecertificationPayload = {
  ...
  return (
    <div
      role="alertdialog"
      aria-labelledby="process-recertification-title"
      aria-describedby="process-recertification-desc"
      className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 px-4"
    >
      <div className="w-full max-w-md space-y-4 rounded-lg bg-white p-6 shadow-xl">
        <header className="space-y-1">
          <h2
            id="process-recertification-title"
            className="text-base font-semibold text-slate-900"
          >
            재인증 요청 처리
          </h2>
          <p id="process-recertification-desc" className="text-xs text-slate-500">
            요청 #<span className="font-medium text-slate-700">{request.id}</span> —{' '}
            {request.club.name}
          </p>
        </header>
        ...
        <label className="block space-y-1">
          ...
          <textarea
            value={actionNote}
            onChange={(event) =>
              setActionNote(event.target.value.slice(0, ACTION_NOTE_MAX))
            }
            rows={4}
