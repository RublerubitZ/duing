 ...
 * 두잉이 실제로 쥐고 있는 것(가입 코드 → 가입 요청 → 승인)을 보여준다.
 *
 * <p>네 칸은 전부 서버가 계산해 내려준 값을 그대로 적는다 — 화면에서 더하고 빼면 서버 정의(활성 코드
  ...
  return (
    <ConsoleCard className="mt-5 p-6">
      ...
      <p className="mt-2 text-[13.5px] leading-relaxed text-charcoal-2">
        외부 모집은 두잉에서 지원서를 관리하지 않습니다. 회원 등록은 가입 코드 → 가입 요청 → 운영진
        승인 절차로 진행됩니다.
      </p>
      ...
      <dl className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
        <Stat label="가입 코드 상태" value={joinLink ? JOIN_LINK_STATUS_LABEL[joinLink.linkStatus] : '코드 없음'} />
