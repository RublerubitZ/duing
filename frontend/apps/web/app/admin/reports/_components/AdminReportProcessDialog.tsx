...
export function AdminReportProcessDialog({ report, isPending, errorMessage, onConfirm, onCancel }: Props) {
  ...
  return (
    <div
      ...
      aria-describedby="process-report-desc"
      className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 px-4"
    >
      <div className="w-full max-w-md space-y-4 rounded-lg bg-white p-6 shadow-xl">
        <header className="space-y-1">
          <h2 id="process-report-title" className="text-base font-semibold text-slate-900">
            신고 처리
          </h2>
          <p id="process-report-desc" className="text-xs text-slate-500">
            신고 #<span className="font-medium text-slate-700">{report.id}</span> — {report.targetLabel}
          </p>
        </header>
        ...
        <fieldset className="space-y-2">
          ...
          <div className="flex gap-4">
            {TERMINAL_STATUSES.map((statusOption) => (
              <label key={statusOption} className="flex items-center gap-2 text-sm text-slate-700 cursor-pointer">
                <input
                  type="radio"
                  name="reportStatus"
                  value={statusOption}
