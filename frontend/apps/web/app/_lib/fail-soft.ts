 ...
 *
 * 홈(/)은 ISR(revalidate 600s)이라 실패를 swallow 하면 위험하다 — 백그라운드 재생성 중
 * 백엔드가 순단(배포 재시작 등)이면 로더가 빈 배열/폴백을 돌려주고 재생성이 "성공"으로
 * 처리돼, **빈 홈이 600초 동안 전역 캐시**된다. 반대로 rethrow 하면 Next 는 재생성 렌더를
 * 실패로 보고 직전 정상 캐시본을 계속 서빙한다(stale 유지) — 사용자에겐 10분 지난 홈이
