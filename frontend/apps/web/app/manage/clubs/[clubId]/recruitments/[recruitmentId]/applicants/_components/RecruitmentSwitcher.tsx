 ...
 * "진행 중"이다(§1-2. 모집 관리 표의 displayStatus 기준과 다른 것은 의도).
 * 외부 폼 모집은 지원자 관리 자체가 없어 전환 대상에서 제외한다.
 *
 * 이동은 next/link — 탭 내비 전례대로 View Transition 을 태우지 않는다.
 */
export function RecruitmentSwitcher({ clubId, currentRecruitmentId }: RecruitmentSwitcherProps) {
  ...
  // 목록 로딩·실패·캐시 미스로 현재 모집을 못 찾으면 스위처를 통째로 숨긴다 — 제목 없는 트리거나
  // 현재 모집이 빠진 목록을 보이느니 기존 화면을 그대로 둔다(fail-open).
  if (currentRecruitment === undefined) return null;
