...
// 가입 이름 금칙어 — 테스트 계정·장난 입력·운영자 사칭용 이름 차단. 소문자 정규화 후 정확 일치 시 거부.
// BE ReservedNamePolicy 와 동일 목록 유지(최종 검증은 백엔드).
export const RESERVED_SIGNUP_NAMES: readonly string[] = [
  '테스트',
  '테스터',
  '관리자',
  '운영자',
...
export const signupSchema = z.object({
  ...
  // 다국어 지원 시 이 정규식만 확장하면 된다. BE SignupRequest 와 동일 정책.
  name: z
    .string()
    .trim()
    .min(1, '이름은 필수 입력값입니다.')
    ...
    .refine(
      (name) => !RESERVED_SIGNUP_NAMES.includes(name.toLowerCase()),
