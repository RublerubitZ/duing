...
describe('slotPatternSchema', () => {
  ...
  it('datetime-local 형식(YYYY-MM-DDTHH:mm) 의 미래 startTime 을 허용한다', () => {
    expect(
      slotPatternSchema.safeParse({
        startTime: futureLocalDateTime(),
        intervalMinutes: 30,
  ...
  it('잘못된 형식 startTime 을 reject 한다', () => {
    expect(
      slotPatternSchema.safeParse({
  ...
  it('과거 시각 startTime 을 reject 한다', () => {
    expect(
      slotPatternSchema.safeParse({
        startTime: pastLocalDateTime(),
        intervalMinutes: 30,
        count: 4,
        capacity: 2,
      }).success,
