...
- 컬럼 추가와 같은 릴리스에서 `DROP DEFAULT`·`SET NOT NULL` 로 제약을 강화하지 말 것 — 배포 실패 시 자동 롤백으로 그 컬럼을 모르는 구 이미지가 다시 뜨는데 Flyway 는 되돌아가지 않아 해당 기능의 INSERT 만 조용히 깨진다(제약 강화는 다음 릴리스에서)
