...
public final class UserFixture {
    ...
    // studentId 가 호출마다 유일하도록 nanoTime 기반 시퀀스를 둔다(기존 인라인 saveUser 전례).
    private static final AtomicLong SEQUENCE = new AtomicLong(System.nanoTime());

    private UserFixture() {
    }

    /** 학번이 호출마다 유일한 학생 User 를 생성한다(저장은 호출 측 repository 책임). */
    ...
    /** 이름을 지정한 학생 User 를 생성한다(학번은 유일). 동명이인·이름 매칭 검증용. */
    ...
    /** ADMIN 역할 User 를 생성한다(학번은 유일). 어드민 권한 통합 테스트용. */
