...
class GeneralClubEventServiceKstWindowTest {
    ...
    void defaultWindowIsAnchoredToKstToday() {
        ...
        GeneralClubEventService service =
                new GeneralClubEventService(eventRepository, userRepository, fixedClock);
        when(eventRepository.findWindow(eq(CLUB_ID), any(), any())).thenReturn(List.of());

        service.listWindow(CLUB_ID, null, null);

        ArgumentCaptor<LocalDateTime> fromCaptor = ArgumentCaptor.forClass(LocalDateTime.class);
        ArgumentCaptor<LocalDateTime> toCaptor = ArgumentCaptor.forClass(LocalDateTime.class);
        verify(eventRepository).findWindow(eq(CLUB_ID), fromCaptor.capture(), toCaptor.capture());
