...
class JoinCodeCreateConcurrencyTest extends IntegrationTestBase {
    ...
    void concurrentCreateLeavesSingleActiveCode() throws Exception {
        ...
        saveOpenExternalRecruitment(club);

        ExecutorService pool = Executors.newFixedThreadPool(2);
        Callable<Throwable> leaderTask = () -> tryCreate(club.getId(), leader.getId());
        Callable<Throwable> officerTask = () -> tryCreate(club.getId(), officer.getId());
        ...
        // 핵심 contract 3: partial unique 가 다중 활성 코드를 구조적으로 막는다.
        Integer activeCodeCount = jdbcTemplate.queryForObject(
                "SELECT COUNT(*) FROM club_join_code "
                        + "WHERE club_id = ? AND revoked_at IS NULL AND deleted_at IS NULL",
                Integer.class, club.getId());
        assertThat(activeCodeCount).as("활성 코드는 정확히 1개").isEqualTo(1);
    }

    private Throwable tryCreate(Long clubId, Long requesterId) {
        try {
            joinCodeService.create(new CreateJoinCodeCommand(clubId, requesterId, 30, 30, 1));
            return null;
    ...
    private void saveOpenExternalRecruitment(Club club) {
        recruitmentRepository.save(Recruitment.createWithOptions(club,
