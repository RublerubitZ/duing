...
class ClubMemberResponseTest {
    ...
    @Test
    @DisplayName("응답으로 변환할 때 전화번호는 마스킹되고 학과·학년·기수·회비상태는 그대로 전달된다")
    void masksPhoneAndCarriesMajorGrade() {
        ClubMemberQuery query = new ClubMemberQuery(
                ...
                "컴퓨터정보공학", Grade.SENIOR, "010-1234-5678", 3, MemberFeeStatus.PAID);
        ...
        assertThat(response.feeStatus()).isEqualTo(MemberFeeStatus.PAID);
    }

    @Test
    @DisplayName("전화번호가 null 이면 phoneMasked 도 null 이고, 기수 미사용 시 generation 은 null 로 유지된다")
    void nullPhoneStaysNull() {
        ClubMemberQuery query = new ClubMemberQuery(
                1L, 42L, "구승율", "20210001", ClubMemberRole.MEMBER,
                LocalDateTime.of(2026, 1, 1, 0, 0),
                "컴퓨터정보공학", Grade.SENIOR, null, null, MemberFeeStatus.NONE);

        ClubMemberResponse response = ClubMemberResponse.from(query);
        assertThat(response.phoneMasked()).isNull();
        assertThat(response.generation()).isNull();
        assertThat(response.feeStatus()).isEqualTo(MemberFeeStatus.NONE);
    }
}
