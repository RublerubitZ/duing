 ...
 *
 * <p>창의 키는 IP 가 아니라 <b>요청자 userId</b> 다. 두 엔드포인트 모두 인증이 필수라 userId 가 항상 확정되고,
 * 교내 NAT 뒤에서는 운영진 여럿이 한 IP 를 공유해 IP 축으로 잡으면 정상 열람까지 함께 막힌다.
 *
 * <p>지원자 번호 열람과 부원 번호 열람은 <b>같은 창</b>을 쓴다 — 지키려는 값이 "한 사람이 긁어갈 수 있는 번호
 * 총량"이라 엔드포인트별로 창을 나누면 합산 상한이 두 배가 된다. 분 30 은 지원자 목록을 1초에 한 명씩 눌러도
 * 30초를 통과하는 값이고, 시 300 은 사실상 스크래핑만 걸린다.
 *
