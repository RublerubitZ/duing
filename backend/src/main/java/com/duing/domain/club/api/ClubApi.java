...
public interface ClubApi {
    ...
    ResponseEntity<ApiResponse<PageResponse<ClubSummaryResponse>>> getClubs(
            ...
            @Parameter(description = "정렬 옵션 (RECOMMENDED / DEADLINE_SOON / ALPHABETICAL / POPULAR). 미지정 시 RECOMMENDED(추천순: 모집중→상시→기타 그룹 + 시간별 순환·활동점수). POPULAR 는 활성 모집 지원자수 → 즐겨찾기수 → 활성 모집 시작일.") @RequestParam(required = false) ClubSortOption sort,
