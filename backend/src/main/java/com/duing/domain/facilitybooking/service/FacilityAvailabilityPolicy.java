 ...
 *
 * <p>전면 차단 설계(2026-08-27): 크롤 행은 분류와 무관하게 전부 차단 대상이다. 분류는 "기본 확보 시간
...
public class FacilityAvailabilityPolicy {
    ...
    /**
     * 분류는 표시·정책 의미 전용이다 — 어느 쪽이든 차단은 {@link #blockingOverlapping} 이 담당하므로
     * 이 판정이 슬롯을 열어줄 수 없다. securedOrganizationKeys 는 호출부가 요청당 1회 조회해 넘긴다.
     */
    ...
    /**
     * 크롤 행이 지정 날짜의 [startTime, endTime) 반개구간과 겹치는 행만 남긴다. 크롤 행은 분류와 무관하게
     * 전부 차단 대상이다(전면 차단 P1 — 미등록·미매칭·기본 확보 모두). 신청 차단·승인 재검증·관리자 상세·
     * 부분반영·충돌의심은 "같은 규칙이어야 하는 쌍"(설계 §5.1↔§5.2)이라 이 필터 하나를 공유한다 —
     * 갈라지면 이중 대관 승인이 난다. 결과 성형(boolean/payload/컨텍스트 누적)은 호출부 소관이므로
     * Stream 을 그대로 돌려준다.
