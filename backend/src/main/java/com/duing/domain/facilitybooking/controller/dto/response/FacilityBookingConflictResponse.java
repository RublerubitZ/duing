 ...
 * conflicts 항목은 source("SCHOOL" 고정)·단체명·시작/끝(HH:mm)으로, crawlBasisAt 은 KST(+09:00) OffsetDateTime 이다.
 * crawlBasisAt 은 스냅샷이 없으면 null 이라 Map.of 로 담을 수 없어 전용 record 로 싣는다.
 */
public record FacilityBookingConflictResponse(List<ConflictSlot> conflicts, OffsetDateTime crawlBasisAt) {
    ...
    public static FacilityBookingConflictResponse from(SchoolConflictException exception) {
        List<ConflictSlot> conflicts = exception.getConflicts().stream()
                ...
                .toList();
        return new FacilityBookingConflictResponse(conflicts, toKst(exception.getCrawlBasisAt()));
    }

    /** crawl_basis_at 은 KST wall-clock LocalDateTime 저장값 — FacilityUsageResponse.toKst 와 동일 변환. */
    private static OffsetDateTime toKst(LocalDateTime crawlBasisAt) {
        return crawlBasisAt == null ? null : crawlBasisAt.atZone(KST).toOffsetDateTime();
    }
}
