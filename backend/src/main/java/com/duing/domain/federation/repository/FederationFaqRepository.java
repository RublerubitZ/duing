...
public interface FederationFaqRepository extends JpaRepository<FederationFaq, Long>, FederationFaqRepositoryCustom {
    ...
    // (b) soft-deleted FAQ는 이관하지 않는다 — 비노출 행이고, 삭제된 카테고리를 참조해도 이름 해석
    //     (categoryNameMap)이 null을 허용하므로 안전하다. @SQLRestriction은 벌크 UPDATE/DELETE HQL에는
    //     적용되지 않으므로(SELECT JPQL만 자동 적용) deletedAt is null 명시가 필수다.
