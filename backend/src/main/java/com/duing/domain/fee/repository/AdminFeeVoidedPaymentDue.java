 ...
 */
public record AdminFeeVoidedPaymentDue(Instant voidedAt, LocalDate dueDate) {

    private static final ZoneId SEOUL = ZoneId.of("Asia/Seoul");

    /**
     * 마감일이 지난 뒤의 정정인가 — 마감 당일은 아직 경과가 아니다(연체 판정과 같은 기준).
     * 마감일(due_date)이 KST 날짜라 정정 시각도 KST 달력으로 환산해 비교한다.
     */
    public boolean isAfterDueDate() {
        return voidedAt != null && voidedAt.atZone(SEOUL).toLocalDate().isAfter(dueDate);
    }
}
