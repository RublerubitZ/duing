...
@Tag(name = "동아리 활동 이력(총동연)",
        description = "총동연 전용 동아리 활동 이력 — 상태 전이·폐쇄·가입 링크(모집 링크·부원 초대) 생성/재발급/폐기/강제 폐기.")
...
public interface AdminClubActivityApi {
    ...
    @Operation(summary = "동아리 활동 이력 (ADMIN)",
            description = "동아리의 상태 전이·폐쇄·가입 링크 이벤트를 최신순으로 반환한다. "
                    + "types 미지정이면 허용 6종(CLUB_STATUS_CHANGED, CLUB_CLOSED, JOIN_LINK_CREATED, "
                    + "JOIN_LINK_REGENERATED, JOIN_LINK_REVOKED, JOIN_LINK_FORCE_REVOKED) 전체, "
                    + "지정하면 허용 집합과의 교집합만이며 "
                    + "전부 허용 밖이면 빈 결과다(회비·가입 요청 종류는 실리지 않는다). "
                    + "recruitmentId 가 null 인 가입 링크 이벤트가 부원 초대 링크이며, joinCodeId 로 가입 링크 목록의 해당 행을 찾는다. "
                    + "JOIN_LINK_FORCE_REVOKED 는 총동연 강제 폐기라 reason 에 사유가 함께 실린다. actorName 은 탈퇴 회원이면 비어 나오고, "
