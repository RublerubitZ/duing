...
가입 요청 생성만으로는 아무것도 확정되지 않는다. **승인 시점에만** 사용 인원을 차감하고 ClubMember 를 생성한다.
...
승인 시 다음을 **단일 트랜잭션**으로 수행한다:

1. 코드 행 비관적 잠금(PESSIMISTIC_WRITE) 후 잔여 확인·차감 — 잔여가 없으면 "잔여 사용 가능 인원 부족" 실패
2. ClubMember 생성 — **기존 공통 서비스**(지원 승인과 동일 로직: upgrade-or-insert, 탈퇴 후 재가입, 동시성 멱등) 호출,
   generation 은 요청의 스냅샷 값 전달
3. JoinRequest 상태 변경 (APPROVED) + reviewed_by/reviewed_at 기록

- **승인 시점에 이미 다른 경로로 활성 ClubMember 가 된 경우**: 해당 요청은 자동으로 REJECTED 처리한다
  (reject_reason = "이미 가입된 회원", 인원 미차감). PENDING 으로 방치하지 않는다.
- 거절: 상태 변경 + reviewed_by/reviewed_at 기록만 수행. 인원 미차감.
...
- 실패 항목은 사유와 함께 반환한다: 잔여 인원 부족 / 이미 가입된 회원(자동 거절됨) / 이미 처리된 요청 등.
