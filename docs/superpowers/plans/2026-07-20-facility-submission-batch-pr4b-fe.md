...
2. **완료 처리 버튼 라벨 = `제출 완료` (사용자 확정)** — 업무 흐름 `제출 목록 만들기 → 학교 제출(오프라인) → 제출 완료` 로 이어지는 명명. 준비 탭 Dialog 본문("'제출 목록' 탭에서 '제출 완료' 처리를 해주세요")과 일치(문구 정정 91f861a2 연동). 스펙 §7.3 의 "학교 제출 완료 처리"는 행위 서술이지 라벨이 아니다.
3. **취소 확인 Dialog 문구 (사용자 확정 — 원문 그대로, 3문단):** 제목 `제출 목록을 취소할까요?`, 본문 ①`취소하면 이 제출 목록은 사용할 수 없게 됩니다.` ②`담긴 예약은 다시 '학교에 제출할 예약' 목록으로 돌아갑니다.` ③`이 작업은 되돌릴 수 없습니다.` (예약까지 사라지는 오해 방지 — 목록 무효화와 예약 복귀를 분리 서술), 확인 버튼 `제출 목록 취소`, 성공 토스트 `제출 목록이 취소되었어요.`
...
5. **목록 페이지 크기 10, facilityId 필터 UI 없음 (사용자 확정 — YAGNI)** — BE 는 facilityId 쿼리를 지원하나 v3 운영 흐름(월 1~2회·소량)에서 필터 수요 미확인. Out of Scope 에 명시.
...
- 문구(스펙 §7.3 원문 — 바이트 단위):
  ...
  - 취소 Dialog: 결정 사항 3 의 확정 문구(본문 3문단 구조 유지 — 한 문단으로 합치지 말 것)
