...
- **두 모드 모두 동일 문구** (모드 분기 제거):
  `"이미지가 보이지 않을 때 대신 보여주거나 읽어주는 설명입니다."`
- 헬프 문구는 Alt Text 의 본질적 역할 (이미지 대체 텍스트) 만 설명한다. SYSTEM 모드에서의 "전환 시 자동 적용" 같은 동작 설명은 헬프 문구에 두지 않는다 — 그건 입력값의 동작이지 필드의 역할이 아니다.
- "스크린리더", "SEO", "접근성" 같은 기술 용어 제거. 일반 사용자가 즉시 이해하는 시나리오 묘사.

### 3.6 메인 캐러셀 컨테이너 비율 (Issue #6)
- `BannerCarousel.tsx:151` 의 `h-[280px]` → `aspect-[16/7]` (Tailwind 기본 지원).
- 컨테이너 비율을 권장 사이즈 (1920×840, 16:7) 와 정확히 일치시켜 `object-cover` 가 **권장 규격(16:7) 기준 잘림 없이** 채우게 한다. 비권장 비율의 이미지는 `object-cover` 특성상 여전히 크롭될 수 있으며, 이는 어드민 폼의 권장 비율 경고(spec 선행 사양 §6.4) 가 사전 안내한다.
- 메인 슬라이드 박스가 ~924×280 → ~924×404 로 세로 ~44% 커짐. 페이지 상단을 더 차지하지만 권장 사이즈 기준 잘림은 사라진다.
...
### 6.2 브라우저 sanity
- 메인 페이지: 컨테이너 비율 변경 후 mock 배너(세로 ~404px) 가 정상 표시, 슬라이드 전환 애니메이션 회귀 없음.
- 메인 페이지: 어드민이 ctaLabel 비워둔 DB 배너에 "자세히 보기" 가 안 보임.
- 메인 페이지: 보조 배너의 부제가 더 이상 화살표와 함께 표시되지 않음.
- 어드민: FULL_BLEED 라디오 토글 시 4개 입력란이 부드럽게 숨김/복구. title 입력란은 항상 보임. Alt Text 헬프 문구가 두 모드 모두 동일 표현으로.
- 어드민: 1896×830 (권장 16:7) 이미지를 업로드한 FULL_BLEED 배너가 메인 페이지에서 권장 규격 기준 잘림 없이 표시.

### 6.3 반응형 레이아웃 캡처 (필수)
`aspect-[16/7]` 적용 후 다음 3종 화면 너비에서 캡처를 첨부해 PR 본문에 함께 올린다 — 레이아웃 영향(상단 차지 비중, 사이드 미리보기 줄어듦, 모바일에서 카드 깨짐 여부) 을 최종 확인.

| 환경 | 권장 viewport |
|------|--------------|
| Desktop | ≥ 1280px (`max-w-layout` 기준) |
| Tablet | 768~1024px (md 브레이크포인트 경계) |
| Mobile | ≤ 640px (sm 미만) |

기존 `md:grid-cols-[1fr_340px]` 가 md 미만에서는 단일 컬럼으로 떨어지므로, 모바일에서 메인 슬라이드가 화면 폭 ÷ 16 × 7 비율로 정상 표시되는지 특히 확인한다.
