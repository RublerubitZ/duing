...
- **D1. 운영 중단 전환 시 OPEN 모집을 자동 마감(CLOSED)하는가?**
  - **추천: 예.** 운영 중단은 "신규 모집 활동 정지"가 자연스럽고, 조회 필터만으로는 "지원 가능해 보이는 OPEN 모집" 데이터가 남아 어드민 화면·통계·향후 쿼리에서 계속 함정이 된다. 재개 시 새 모집을 열면 된다 (Out of Scope 참조).
  - 대안: 조회 필터만 적용하고 모집은 OPEN 유지 — 재활성 시 모집이 되살아나는 게 장점이지만, 마감일이 지난 채 부활하는 등 상태 불일치 함정이 더 크다.
- **D2. 자동 마감 시 진행 중(SUBMITTED 등) 지원서 처리는?**
  - **추천: 미변경(그대로 둔다).** 폐쇄 cascade 는 지원서를 REJECTED 로 강제 전환하지만, 운영 중단은 되돌릴 수 있는 상태라 지원서까지 파괴적으로 처리하지 않는다. CLOSED 모집의 지원서 열람·처리 정책은 기존과 동일.
...
- **D3. 비 ACTIVE 동아리의 기존 멤버 내부 영역 접근을 허용하는가?**
  - **추천: 전면 차단 (`requireActiveMember` 도입).** 근거: ① 정책 단순 일관 — "학생에게 비 ACTIVE 는 어떤 형태로도 노출 금지"의 자연 연장 ② 정상 UI 진입 경로가 이미 없어 실사용자 피해 없음 ③ 재활성 시 자동 복귀. 특히 회비 계좌는 민감도상 즉시(P1) 차단.
  - 대안: 공지·일정 등 읽기는 "과거 기록 열람권"으로 허용하고 쓰기·민감(회비)만 차단 — 멤버 친화적이지만 엔드포인트별 예외 매트릭스가 생겨 유지보수 비용 증가.
- **D4. 차단 시 응답 코드는?**
  - **추천: 403 + "운영 중단된 동아리입니다" 계열 메시지.** 본인이 소속했던 동아리라 존재 은닉(404)이 무의미하고, 사용자에게 이유를 알려주는 편이 낫다. (공개 표면의 404 존재 은닉과 구분되는 지점 — 멤버는 이미 존재를 안다.)
...
- **D5. 어떤 쓰기까지 ACTIVE 를 요구하는가?**
  - **추천: "운영 행위"만 ACTIVE 전용, "프로필 보완"은 전 상태 허용(현행 유지).**
    - ACTIVE 전용(운영 행위): 모집 CUD, 공지 작성, 이벤트 CUD, 회비 설정/청구, 면접 라운드, 멤버 관리(역할 변경·제명·승계), CSV 내보내기
    - 전 상태 허용(프로필 보완): 동아리 정보 수정, 사진 CUD, SNS 링크 — 근거: 재심사 흐름이 "동아리장이 신청 내용을 보완한 뒤 재심사"를 전제하므로(총동연 콘솔 재심사 버튼 설명 문구), PENDING_APPROVAL/REJECTED 에서 정보·사진 보완은 막으면 안 된다.
  - 대안: 전면 ACTIVE 요구 — 재심사 보완 흐름이 깨지므로 비추천.

### 설계 (D5=추천안 기준)

1. `ClubAuthService` 에 `requireActiveManager` / `requireActiveLeader` 추가 (Part B 의 `requireActiveMember` 와 동일 패턴, 403).
2. 운영 행위 엔드포인트의 기존 `requireManager`/`requireLeader` 호출을 교체 — 구현 시 호출처 전수 grep 으로 "운영 행위 vs 프로필 보완" 분류표를 계획서에 확정하고 리뷰에서 검증.
...
- 통합: INACTIVE/REJECTED 동아리 리더의 모집 생성 → 403, 동아리 정보 수정·사진 업로드 → 기존 허용 유지
- 통합: PENDING_APPROVAL 리더의 정보 수정 허용 (재심사 보완 흐름 회귀 방지)
