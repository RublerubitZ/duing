...
7-3. **수용된 잔여 윈도우 (경계 응답, BE#9 설계 판정)**: 응답 TX 가 기간 검사(COLLECTING)를 통과한 직후 자동배정이 전이·배정·커밋하면, 그 마지막 순간 응답 1건은 draft 에 미반영될 수 있다. availability·RESPONDED 데이터는 일관하므로 모순이 아니며, ASSIGNING 재실행과 확정 게이트(§6.3 respondedUnassigned)가 노출·흡수한다 — 응답 측 round 공유 잠금 도입은 과설계로 보류. ASSIGNING 진입 후 응답은 기존대로 409. 같은 부류로, 마감 연장(BE#12) 커밋 직전 경계에서 응답 1건이 직전 마감 기준으로 409 를 받을 수 있다 — 연장만 허용 정책 덕에 반대 방향(닫혔어야 할 응답 수용)은 불가능하고, 재시도가 자연 회복하므로 수용한다.
