...
7-1. **슬롯 변경과 응답의 직렬화 (BE#4 리뷰 반영, BE#8 요구사항)**: 슬롯 시간변경·삭제의 "선택 참조 0" 검증은 read-check-then-write 라서, 응답 API(BE#8)가 availability 를 삽입할 때 **대상 슬롯 행 잠금(또는 삽입 후 슬롯 활성 재검증)** 으로 직렬화하지 않으면 검증 직후 선택된 슬롯이 변경·삭제될 수 있다.
