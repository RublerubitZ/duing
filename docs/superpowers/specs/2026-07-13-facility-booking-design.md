...
**불변식**
- CONFIRMED에서 나가는 전이는 없다(관리자 포함). 예외가 필요해지면 P3에서 별도 설계.
...
- 셀 채움: BLOCKED=단체명 약칭 채움(ink-soft/회색), PENDING_HOLD=점선 테두리, AVAILABLE=빈 칸. 주간 뷰에서도 셀 탭 = 그 날짜·슬롯 선택(선택일 자동 전환).
