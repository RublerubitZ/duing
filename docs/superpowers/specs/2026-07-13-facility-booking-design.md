...
- CTA 탭 → 시트 안에서 신청 폼 단계로 전환(시트 유지, 뒤로 가능): 동아리 선택(운영진 동아리 1개면 자동 고정 표시), 사용 목적(필수 — 아래 Preset + 자유 입력 하이브리드), 사용 인원(필수 — 2026-07-21 정책 변경, 학교 제출 양식이 인원을 요구해 선택 → 필수로 개정). 폼 필드는 MVP 최소 구성이되, 학교 전달 양식 변경에 대비해 폼 스키마(필드 정의·검증)를 한 모듈에 모아 필드 추가가 국소적이게 한다. PENDING_HOLD 포함 시 폼 상단에 "이미 예약 신청이 접수된 시간입니다. 계속 신청하시겠습니까?" 경고 블록.
...
15. 신청 폼 = 사용 목적(필수) + 사용 인원(필수, 2026-07-21 개정 — 학교 제출 양식 요구로 선택 → 필수). 학교 전달 양식 변경에 대비해 필드 추가가 국소적인 구조 유지.
