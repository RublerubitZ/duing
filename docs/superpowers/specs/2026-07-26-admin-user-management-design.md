...
> `admin_user_action_log`는 보존기간이 없는 영구 테이블이고(결정 D-17), 탈퇴 회원 개인정보 파기 잡의 대상도 아니다. `users.admin_note`가 익명화 대상인 것과 대조적이다. 영구 보존을 감수하는 대신 개인정보 유입 자체를 막는 운영 원칙으로 완화한다.
>
> **작성 원칙의 원본은 총동아리연합회 운영 가이드(계정 정지·해제 절차)에 있다.** 실제로 사유를 입력하는 사람은 이 문서가 아니라 그 문서를 읽으므로, 좋은 예/나쁜 예를 포함한 본문은 그쪽에서 관리하고 여기서는 중복해 적지 않는다 — 두 곳에 두면 갈라진다.
>
> 이 원칙은 화면 안내 문구로는 노출하지 않는다 — 입력 폼에 "개인정보를 쓰지 마세요"를 띄우면 무엇을 쓰면 안 되는지만 알려주게 되어 오히려 무엇을 써도 되는지가 흐려지고, 사유를 필수로 받는 목적(책임 추적)과 반대 효과가 난다.
...
| D-17 | 감사 로그 `reason`은 영구 보존 — 파기 대상으로 만들지 않음 | 조치 근거가 사라지면 감사 로그의 존재 이유가 없어진다. 대신 `reason`에 개인정보를 쓰지 않는 운영 원칙으로 유입 자체를 막는다 — 작성 원칙 본문은 총동연 운영 가이드가 관리한다(§계정 상태 변경 참조) |
