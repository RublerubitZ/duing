...
키워드까지 태우지 않는다(단과대학은 DB 에 enum 코드로 있어 한글 검색하려면 코드↔라벨 매핑을
쿼리로 끌고 들어가야 한다).

**총동연 목록(`/admin/clubs`)에도 함께 적용된다** — 학생·총동연 목록이 같은 `keywordContains`
술어를 공유하기 때문이다. 동아리명·소개·태그도 원래 같은 방식으로 양쪽에 걸려 있었으므로 기존
관례를 따르는 것이고, 학과는 이미 카드에 공개되는 정보라 새로 노출되는 것도 없다.
