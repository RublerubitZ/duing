...
중앙=분과, 단과대=학과로 갈리는 규칙을 헬퍼 하나로 모아 `ClubCard`·`ClubListItem`이 공유한다.
값이 없으면 `null`을 반환해 **영역 자체를 렌더하지 않는다**(placeholder 금지).
...
단과대 이름은 카드에 표기하지 않는다 (배지가 유형, 보조 텍스트가 학과).

### 6.3 상세 페이지

- 히어로 pill: `카테고리 · 분과`(중앙) / `카테고리 · 학과`(단과대).
  현재 비중앙에서도 `division` 잔여값이 새어나오던 것을 `centralClub` 게이트로 함께 막는다.
...
- **FE**: 카드가 단과대 동아리에 `[단과대]` + 학과를 그린다 / 학과 없으면 영역 미렌더 /
  운영진 폼에서 학과 편집 가능·단과대는 잠금 / 관리자 생성 폼 단과대 필수

## 8. Out of Scope

- 학과명 표준화·자동 정규화·학과 마스터 데이터 (자유입력만)
- 학과 기준 검색/필터 신규 추가 (기존 단과대 필터만 유지)
- 기존 `division` 잔여 데이터 정리·삭제
- 총동연 동아리 **수정** 화면 신규 구축 (현재 UI 없음 — API로만 지원)
- 총동연 목록/상세 테이블에 학과 컬럼 추가
- `centralClub` 토글 API의 college 필수 검증 — 결정 5(보존) 우선. 중앙 → 단과대 전환으로
  `college IS NULL` 인 단과대 동아리가 생길 수 있고, 이는 기존 NULL 행과 동일하게 취급한다
