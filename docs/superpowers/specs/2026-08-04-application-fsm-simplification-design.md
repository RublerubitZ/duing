...
새 Flyway 파일 **V100**(가입 코드 시리즈가 V99 까지 진행 중인 것을 확인해 V100 으로 확정) 하나로 값 치환만 수행한다. CHECK 제약·시드가 없어 DDL 불필요.
...
- **롤백 정책 (V95 치환 전례와 동일 패턴)**: 마이그레이션 직후의 BE 롤백은 안전 — 치환 결과(SUBMITTED)는 구 FSM 에서도 유효한 값. 단, **신버전 운영 중 ON_HOLD 데이터가 생성된 뒤 이전 이미지로 롤백하려면 역치환 UPDATE(`ON_HOLD → 'SUBMITTED'`, application + history 양쪽) 선행 필수** — 구 enum 이 ON_HOLD 를 역직렬화하지 못한다. 또한 **롤백 창 동안 구 코드가 새로 만든 UNDER_REVIEW 행은 재배포 시 V100 이 재실행되지 않아 그대로 남는다 — 재배포 전에 V100 의 치환 UPDATE 3문을 수동 재실행해 정리한다(멱등).** 방치하면 상수 제거(§1-4) 이후 코드에서 해당 행 로드 시 500 이 고착된다.
