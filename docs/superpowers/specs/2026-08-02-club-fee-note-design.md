...
- **상세 페이지** (`/clubs/[id]`):
  ...
  - `ClubDetailStats`(통계 카드): 회비 셀 아래 최대 2줄 보조 텍스트로 안내문 표시(사용자 결정 2026-08-02).
    셀 노출 게이트는 그대로 **대표 금액이 있을 때만** — 대표 금액 없이 안내문만 있는 경우는 상세정보 탭이 커버한다.
    2줄을 넘는 안내문은 말줄임 부호 없이 잘림(전문은 상세정보 탭)
...
- `ClubDetailStats` 카드에서 대표 금액 없이 안내문만으로 회비 셀을 만드는 것
- 모집공고 등 다른 화면의 회비 표기
