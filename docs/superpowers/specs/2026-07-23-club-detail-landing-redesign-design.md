...
- **모바일(<md) — 개정**: 3열이 아니라 **세로 행 리스트** — 각 항목 = flex 행(라벨 고정폭 좌측 + 값 우측 전폭). 긴 활동 값도 한 줄로 표시되고 최악 케이스만 전폭에서 자연 개행(1차 시도였던 "3열+타이포 축소"는 123px 컬럼에서 2줄 꺾임이 여전히 어색해 실QA로 기각). 값 `text-[15px]`·`break-keep [overflow-wrap:anywhere]` 유지.
- **Desktop(md+) 무변경**: 3열·값 22px 기존 그대로(md:grid-cols-3 복원).
