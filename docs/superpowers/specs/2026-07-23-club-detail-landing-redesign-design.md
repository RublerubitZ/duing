...
## v3 확장 (같은 PR) — 소개 카드·소식 탭·소개글 리치 에디터

### A. 소개(About) 카드 리디자인
- Paper Card(radius 20·shadow-1·p-7)로 재작성. 내부: 본문 → (하단 선택 영역) highlights 체크 칩.
- **더보기 기준(확정)**: 첫 문단 항상 노출, 이후 문단은 펼침 영역(grid-rows 0fr→1fr 트랜지션, 펼침부 상단 border). 문단 구분 없으면 4줄 클램프 후 더보기. 펼칠 내용 없으면 버튼 숨김.
- **highlights**: 소제목 없이 카드 하단 ✓ 칩 행. 데이터 없으면 영역 자체 숨김("이런 사람이 좋아할 거예요" 제목·리스트 제거).

### B. 소식 탭 (공지+일정 통합, 멤버 전용)
- 공지·일정 탭 2개 → **"소식"** 탭 1개. 레이아웃: PC 2열(최근 공지 | 다가오는 일정), 모바일 세로(공지→일정).
- 최근 공지: 카드 리스트 — 카테고리 pill 배지 + 제목 + 작성일. 다가오는 일정: 날짜 카드(일+요일) | 세로 구분선 | 일정명 + 시간·장소. 기존 컴포넌트의 기능 인벤토리(로딩·빈 상태·페이지네이션/링크) 보존, 디자인만 목업 기준.
- 데이터·API 무변경(기존 notice/event 쿼리 훅 그대로).

### C. 소개글 리치 에디터 (콘솔)
- **기존 Tiptap 에디터(NoticeRichEditor) 재사용** — 별도 에디터 금지. 기능 구성 prop 으로 소개용 제한: 허용 = 문단·굵게·기울임·취소선·링크·글머리·번호 목록·인용·구분선. 제외 = 제목(h2/h3)·이미지·색상·표·코드. 공지 소비처는 기본값으로 무변경.
- **글자 수**: 텍스트 기준 최대 1,500자(에디터 실시간 카운터 "N/1,500 · 권장 300~800자", 초과 시 저장 불가 인라인 에러). zod description 한도는 HTML 백스톱으로 상향(클럽 스키마 2000→10000, 텍스트 한도는 폼 레이어가 강제). **BE 무변경 확인됨**(UpdateClubRequest @Size 없음·컬럼 TEXT).
- **학생 렌더**: 공지 sanitizeHtml 재사용(허용 태그가 정책 서식 커버 — 에디터가 h2/img 를 못 만들므로 과잉 허용은 무해) + React19 주입 서브트리 memo(레포 함정). HTML 분할은 sanitize 후 최상위 블록 단위(첫 블록=항상 노출).
- **레거시 전환**: 기존 plain text description 은 `^\s*<` 휴리스틱으로 감지 — 학생 렌더는 pre-wrap+기존 분할 규칙, 콘솔 편집 진입 시 빈 줄 기준 `<p>` 변환 시드(개행 소실 방지). 신규 저장은 HTML.

### v3 Out of Scope 추가
- 공지 에디터·공지 상세 화면 변경, BE 검증 추가, description 일괄 마이그레이션(레거시는 휴리스틱 전환).

---

## v4 개선 (같은 PR) — 소개 섹션 통일

- **섹션 헤더**: 소개 카드 위에 다른 랜딩 섹션과 동일 양식(Section Header + Sub) — 제목 "소개" / 서브 "동아리가 추구하는 문화와 활동 방식을 소개합니다." 랜딩 흐름(대표 활동 → 이런 활동을 해요 → 소개) 3섹션 헤더 디자인 통일.
- **카드 내부 순서(단일 Paper Card)**: 소개글 → 더보기/접기 → Divider → **"이런 분께 추천해요"**(소제목) → 체크 리스트. 더보기 기준은 v3 그대로(첫 문단 상시·이후 펼침·무문단 4줄 클램프·펼칠 것 없으면 버튼 숨김).
- **이런 분께 추천해요**: highlights 를 v3 의 무제목 칩 행에서 **소제목 + ✓ 체크 리스트(세로)** 로 변경. 데이터 없으면 영역+Divider 미렌더. Divider 는 본문·추천 둘 다 있을 때만.
- 데이터 모델·API 무변경.

---

## v5 개선 (같은 PR) — 우측 신청 패널 Sticky + 찜 버튼 이동

- **Sticky (Desktop 전용)**: 우측 패널 전체(모집 카드+연락처 카드 묶음 컬럼)를 `lg:sticky lg:top-6 lg:self-start` 로 고정 — **`self-start` 필수**(grid 기본 stretch 가 sticky 를 무력화하는 C-1 전례). 부모 컬럼 끝에서 자연 해제. 학생 GNB 는 비고정이라 top-6 겹침 없음(확인됨). 모바일/태블릿(<lg)은 무변경 — 하단 고정 CTA UX 유지.
- **찜하기 이동**: 모집 카드 하단 버튼 행의 FavoriteToggleButton 을 제거하고 **카드 우측 상단 플로팅**(absolute, 카드 padding 기준 우상단)으로 이동. 모집 상태 무관(모집중·예정·마감 카드 전부) 동일 위치. 기능(토글·aria-pressed) 무변경.
- QA: 스크롤 추종·부모 이탈 없음·긴 소개/Q&A·리사이즈, 찜 토글 동작 유지.

---

## v6 개선 (같은 PR) — 모집 정보 요약(Stats) 모바일 가독성

- **Desktop(md+) 무변경**: 3열·값 `text-[22px]` 기존 그대로.
- **모바일(<md)**: 3열 유지(그리드가 높이 균일 보장)하되 값 타이포 축소 — `text-[15px] leading-snug`(md 에서 22px 복원) + `gap-x-3` 셀 간 여백 + 셀 상단 정렬.
- **개행 안전성(핵심)**: 값에 `break-keep [overflow-wrap:anywhere]` — 공백 있는 한국어는 어절 단위로 자연 개행, 최악의 무공백 토큰("(월·화·수·목·금·토·일)")만 필요 시 강제 분절되어 어떤 길이에서도 오버플로/레이아웃 붕괴 없음. 특정 데이터 길이 의존 없음.
- QA 데이터: 활동 주 2회(수·금)/주 3회/주 5회/주 7회(월~일 전체), 창설년도 2002, 회비 15,000원 — 전 케이스 레이아웃 유지.
