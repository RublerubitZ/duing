...
/forgot-password → 학번 입력 → POST /auth/password-resets
  ...
  · 계정 미존재·탈퇴·번호가 placeholder → 학번에서 HMAC 파생한 decoy 번호로 동일하게 세션 생성(targetUserId=null)
    → 응답 형태·상태코드 모두 위와 동일(균일 202)
...
- 계정 열거: **균일 202로 차단한다**(2026-08-23 정정 — 종전에는 미존재 400을 "수용 리스크"로 두었으나 감사에서 실제 열거가 확인되어 정책을 뒤집었다). 미존재·탈퇴·placeholder 번호 계정은 학번에서 HMAC 파생한 decoy 번호로 **실제 세션을 발급**해 후속 폴링·60초 쿨다운·리밋·QR 왕복시간까지 동일하게 만든다 — 가짜 토큰만 만들면 폴링 첫 틱의 404로 열거가 복원된다. IP 발급 창은 `issue()`가 단독으로 기록하고 시작 지점은 **검사만** 한다 — 검사를 학번 창 기록보다 앞에 둬야 IP 한도로 거절될 요청이 학번 엔트리를 설치하지 못한다(설치되면 8자리 학번 공간만큼 맵이 자란다). 양쪽에서 기록해도 네 분기가 모두 `issue()`를 타므로 소모량은 균일해 열거 오라클은 생기지 않지만, 재설정 실효 IP 예산이 시간당 600→300으로 반감된다. 학번당 시간당 3회 리밋은 그대로 유지한다.
- 레거시 예외: phone이 더미(`010-0000-0000`)인 회원은 여전히 재설정 불가(번호 소유자가 없음) → 운영자 수동 채널. 다만 이들도 decoy 경로로 보내 마스킹 결과가 `010-****-0000` 고정값으로 새지 않게 한다 — 그 값 자체가 "레거시 실계정"임을 알리는 신호이고, `uk_phone_verifications_phone`에는 placeholder 예외가 없어 실번호로 발급하면 placeholder 계정끼리 세션 행을 공유해 서로의 재설정을 쿨다운으로 막는다.
...
| 축 | 정책 | 구현 위치 |
|---|---|---|
| IP — 인증 시작 | 분 60 / 시 600 (남용 백스톱) | `PhoneVerificationRateLimiter` |
...
**2026-08-30 축 전환**: 상태 조회 한도(분 30/시 200)는 값의 근거가 "1인이 폴링하는 양"인데 키를 IP 로 잡고 있었다. 교내 WiFi·통신사 CGNAT 처럼 다수가 한 IP 를 공유하는 환경에서는 정상 가입자끼리 서로의 창을 소모해, 같은 IP 뒤에서 동시 3명·시간당 20명이 실효 가입 상한이 됐다(1인 폴링 ≈ 40초 창에 10회). 같은 한도를 세션(token) 축으로 옮기고 IP 축은 남용 백스톱으로만 넓혔다. 인증 시작 창도 같은 이유로 넓혔다 — "발급은 1인 1회면 충분" 이라는 종전 논증은 한 사람의 재시도를 다룰 뿐 한 IP 뒤의 N명을 다루지 않는다. 1인 단위 통제는 번호별 60초 쿨다운과 (번호,IP) 시간당 5회가 맡는다. 대가로 단일 IP 가 전역 일일 쿼터를 태우는 속도가 빨라지며(`qr=true` 는 요청당 벤더 콜 1건), 그 완화는 이 창이 아니라 **엣지 레이트리밋 도입**(현재 `deploy/Caddyfile` 에 미설정)과 벤더 플랜 조정이 맡는다.
...
- 계정 열거: 가입 중복 409 필드 미특정(기존), 로그인 타이밍 평탄화(기존 이식), 재설정은 균일 202 + decoy 세션으로 차단(§10.2, 2026-08-23)
