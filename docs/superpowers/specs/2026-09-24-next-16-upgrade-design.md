...
4. `next.config.mjs`: (a) `agentRules: false` 추가 — Next 16 은 AI 코딩 에이전트 환경에서 `next dev` 가 `apps/web/AGENTS.md`·`CLAUDE.md` 를 자동 생성한다(`ensureAgentRulesForDev`). 이 레포는 `frontend/AGENTS.md`·`frontend/CLAUDE.md` 를 직접 관리하므로 중첩 파일이 생기면 안 된다. (b) Sentry `webpack.autoInstrumentMiddleware: false` 블록 제거. Turbopack 빌드에서는 무효이고 Sentry 는 Turbopack 에서 미들웨어를 애초에 감싸지 않는다(기존 주석이 이미 그렇게 적고 있음). 죽은 설정을 남기면 3am 에 오독한다. (c) Sentry bundleSizeOptimizations 는 webpack 전용이라 Turbopack 에서 무효 → 같은 매직 플래그를 `compiler.define`(client·server·edge 공통 적용, `defineServer` 중복 키는 빌드 에러 E689)로 직접 치환(실측: 클라이언트 browserTracingIntegration 1→0 파일).
