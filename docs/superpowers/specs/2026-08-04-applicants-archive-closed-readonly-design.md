...
- ~~면접 도메인의 마감 후 심화 대응 — 라운드 내부 쓰기의 CLOSED 가드는 다루지 않는다~~ → **2026-08-05 완료**: 라운드 수정·발송·리마인드·슬롯·배정·확정에 가드를 걸고 리마인더 잡도 마감 모집을 제외했다. 취소만 정리 행위로 허용.
...
> **[2026-08-05 개정] 마감 후 최종 결과 확정은 허용한다.**
> 최초 설계는 "마감 = 전면 조회 전용"이었고, 그 전제는 *정상 흐름상 마감은 심사 완료 후*라는 것이었다.
> 전수조사 결과 그 전제가 성립하지 않는 경로가 확인됐다 — 마감을 유발하는 6개 트리거 중 **신규 모집 등록 시
> 자동 마감·총동연 강제 마감·replaceActive·동아리 운영 중단** 4개는 자동이거나 제3자가 일으키고, 그중
> 절반은 사전 고지도 없었다. 그 결과 마감 시점에 미결이던 지원은 **운영진도 처리 불가(409), 지원자도
> 철회 불가(409), 모집 삭제로 정리도 불가(409)** 인 교착에 빠져 지원자가 결과를 영영 받지 못한다.
>
> 대안으로 검토한 "마감 시 미결 자동 불합격"(동아리 폐쇄 경로의 `rejectActiveOnClubClosure` 전례)은
> 위 자동 트리거 때문에 기각했다 — 새 모집을 하나 등록했다고 심사 중이던 지원자 전원이 본인도 모르게
> 불합격으로 확정되고 되돌릴 수 없다. 대신 **미결 → 최종 결과(합격·불합격) 확정만** 여는 좁은 예외를 둔다.
> 판정은 `ClosedRecruitmentPolicy` 한 곳이 소유한다 — 면접 라운드 운영(수정·발송·확정·슬롯·배정)도
> 같은 정책을 경유하도록 함께 묶었다. 마감된 모집에서 라운드를 굴려 학생에게 면접 알림이 나가는 것을
> 막지 않으면 "결과를 반영하려는 순간에야 막히는" 모순이 남기 때문이다.

`recruitment.status === CLOSED` 인 모집은 **아카이브** — 조회(지원자 목록·지원서·상태·이력·통계)와
**남은 지원서의 최종 결과 확정**만 허용하고 다음을 차단한다:

| 차단 대상 | 행위자 |
|---|---|
| 지원 상태 변경 중 **최종 결과가 아닌 전이**(보류·면접 대상 등, 단건·벌크) | 운영진 |
| 평가(evaluation) 저장 | 운영진 |
| **평가 삭제 (`deleteMine`)** | 운영진 (문서 리뷰 발견 — 읽기 전용 화면에서 도달 가능한 파괴적 쓰기, "조회만 허용" 원칙의 귀결로 차단) |
| 면접 라운드 운영 — 생성·수정·발송·리마인드·슬롯·배정·확정 (**취소는 제외** — 정리 행위라 마감 후에도 허용) | 운영진 |
| **지원 철회** | 지원자 (사용자 확정 — 아카이브 데이터 보존 우선. 모집 마감은 공개 정보라 차단 사유 노출 무해) |

지원 데이터는 어떤 경로로도 삭제·변형되지 않는다 (기존 원칙 유지 — 삭제 로직 자체가 없음).

**허용되는 유일한 쓰기 (2026-08-05 개정)**

| 허용 | 행위자 | 비고 |
|---|---|---|
| 미결(`SUBMITTED`·`ON_HOLD`·`INTERVIEW_PENDING`) → `ACCEPTED`·`REJECTED` | 운영진 | 벌크도 건별로 같은 경로를 경유해 동일 규칙 |

- **면접 모집도 면접 단계를 강제하지 않는다.** 마감 후에는 라운드 생성 자체가 막혀 있어 면접 단계를
  요구하면 미결 지원자에게 불합격밖에 줄 수 없다("면접은 못 봤지만 서류로 합격"이 불가). 그래서 마감
  정리 전이에 한해 `SUBMITTED`·`ON_HOLD` → `ACCEPTED` 직행을 허용한다. 구현은 `useInterview=false`
  위장이 아니라 `Application.transitionTo(.., recruitmentClosed)` 인자로 정책을 코드에 드러낸다.
- **이미 결과가 난 지원의 번복은 계속 불가** — 상태 머신이 종결 상태에서의 전이를 원래부터 막는다.
...
- 지원현황 목록 *(2026-08-05 개정)*: 상단 배너 **"마감된 모집 — 남은 지원서의 최종 결과만 확정할 수 있습니다."** + 체크박스·일괄 처리 바는 **유지**(마감 후에도 정리해야 하므로), 단 일괄 액션은 합격·불합격만 남기고 보류·면접 대상 선정은 감춘다. 필터·검색·목록·통계 링크는 그대로.
- 지원자 상세 *(2026-08-05 개정)*: `StatusActionBar` → 합격·불합격 버튼만 남기고 안내 문구를 덧붙인다("마감된 모집이라 최종 결과만 확정할 수 있습니다…"). 이미 결과가 확정된 지원은 버튼 없이 안내만., `EvaluationPanel` 입력 비활성 + 동일 안내, **`MyEvaluationCard` 의 평가 삭제 버튼 숨김**(§1-3 차단 표 대응), 타임라인·지원서·프로필 조회 유지. 면접 카드의 [면접 관리] 링크는 유지(조회 성격).
...
- BE: 가드 각각 — CLOSED 에서 409 (단 상태 변경은 **최종 결과만 허용**이라 짝 테스트로 고정: 미결→종결 200 / 그 외 409)(+code `RECRUITMENT_CLOSED`) + OPEN(마감일 경과 포함)에서 정상 동작 + CLOSED 에서 조회·통계는 정상. 벌크는 CLOSED 모집 건이 `failures[]` 로 떨어지는 부분 실패 케이스. closedAt 노출(응답 필드·null 허용). `@DisplayName` 요구사항 문장·상대 날짜.
