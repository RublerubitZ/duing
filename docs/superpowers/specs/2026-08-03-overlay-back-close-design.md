...
state 를 보존하지 않는다.** `preserveCustomHistoryState` 는 restore(traverse) 에서만 true 이고
navigate·refresh·server-action·server-patch 리듀서는 전부 false 라, state 가 `{__NA, tree}` 로 통째
교체된다. 즉 **오버레이가 열린 채 `router.replace` / `router.refresh` / 서버액션이 일어나면 그 오버레이
엔트리의 마커가 증발**한다. 그러면 엔트리를 회수할 수도(코드 닫기 가드 실패), 죽은 엔트리로 인식할 수도
없어 죽은 뒤로가기가 계속 누적된다.

대응: `install()` 에서 `replaceState` 를 감싸, **우리 최상단 엔트리 위에서 일어나는 replace 에 한해
마커를 다시 얹는다.** Next 패치와 설치 순서에 무관하게 동작한다(어느 쪽이 바깥이든 통과).
...
설치(`installBackDismiss()`)는 **문서 로드 시점**에 `providers.tsx` 에서 1회 호출한다
(`installBackNavigationViewTransitionGuard()` 와 같은 자리·같은 패턴). 오버레이를 여는 순간까지 미루면,
새 문서로 들어와 오버레이를 한 번도 열지 않은 사용자는 이전 문서가 남긴 엔트리를 만나도 스킵하지 못하고
잔존 마커 제거도 일어나지 않는다(실브라우저 QA 에서 드러난 틈).

리스너는 **1회 등록하고 문서 수명 동안 유지**한다. 스택이 빌 때마다 해제하면
...
| 시트 안에서 `router.replace` 로 URL 변경 | 마커 재주입으로 회수 가능. 닫을 때 바뀐 URL을 착지 엔트리에 다시 얹어 **필터가 초기화되지 않는다** |
| 앞으로 가기로 죽은 엔트리에 올라옴 | 자동 스킵하지 않는다(ID 단조 증가로 방향 판별) — 스킵하면 그 위 엔트리에 영원히 도달 못 함 |
| `onClose` 가 열린 뒤에 뒤늦게 붙음 | 존재 여부를 effect 의존성에 넣어 그 시점에 엔트리를 등록 |
...
- **알림 시트 항목 탭 / 드로어 메뉴 탭** — 닫기와 페이지 이동이 동시에 일어나는 경로(수용 한계 2).
  로그인이 필요해 로컬 브라우저 QA 에서 검증하지 못했다.
- **전송 중(닫기 거부) 시트 + 죽은 엔트리** — 거부 재push 와 스킵 `back()` 의 커밋 순서는 브라우저마다
  다를 수 있다. jsdom 에서는 traversal 이 먼저 소화돼 정상이었으나 실기기 확인이 필요하다.
