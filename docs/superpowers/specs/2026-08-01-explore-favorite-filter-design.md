...
- 하트 아이콘 + "찜한 동아리" 토글 칩. **데스크탑은 상시 노출**(정렬 select 가 있는 목록 상단 행), **모바일은 필터 바텀시트 안 첫 그룹**("찜")에 둔다. 정확한 배치는 구현 시 실브라우저로 확인. *(개정 2026-08-02: 원안은 모바일도 상시 노출이었으나 사용자 결정으로 바텀시트 내부로 이동)*
- 데스크탑 활성 필터 칩 행(`ActiveFilterChip`)에는 **포함하지 않는다**(칩 자체가 상태 표시). 모바일 필터 개수 배지에는 **포함한다** — 배지는 바텀시트 내부 필터 개수라는 의미이고, 칩이 시트 안으로 들어갔으므로 배지에 세는 것이 일관적이다. *(개정 2026-08-02, 같은 결정의 귀결)*
...
- 토글 settled 후 동아리 목록 쿼리를 무효화해 총 개수·페이지 구성을 서버와 동기화한다. *(구현 확정: 무효화는 페이지가 아니라 `useFavoriteToggleMutation` 훅 레벨에서 `clubQueryKeys.all` 프리픽스로 — 목록이 찜에 의존하는 것(찜 필터·POPULAR 정렬 tier)은 이 페이지만의 사정이 아니고, 키 구조상 `list` 만 정밀 타겟할 세그먼트가 없다. 부작용은 상세 화면 하트 토글 시 활성 detail 계열 쿼리 재검증 수준으로 무해 — 실브라우저 QA 확인)*
