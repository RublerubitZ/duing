...
#### GET 인데 감사 로그를 남기는 이유

조회 동사는 부수효과가 없어야 한다는 통념과 충돌해 보이지만, 여기서 남기는 것은 **도메인 상태가 아니라 관측 기록**이다. 로그는 애플리케이션 밖의 관측 채널이고 DB 상태를 바꾸지 않으므로, 이 엔드포인트는 여전히 멱등하고 여러 번 호출해도 같은 결과를 준다. HTTP 의미론상 문제가 없다.

감사가 필요한 이유는 동사가 아니라 대상이다. **개인정보 원본 열람 자체가 기록할 가치가 있는 행위**다. 사고가 났을 때 "누가 언제 누구 번호를 봤는가" 를 되짚을 수 없으면 마스킹 정책은 형식만 남는다. 읽기라서 기록하지 않는다면, 정작 유출 경로인 열람이 통째로 사각지대가 된다.

다만 이 논리는 P1 의 "로그만 남긴다" 에서만 성립한다. P2 에서 전용 테이블에 INSERT 하게 되면 GET 이 DB 쓰기를 유발하므로, 그때는 조회 서비스와 감사 저장을 분리해 별도 쓰기 트랜잭션(또는 비동기 이벤트)으로 처리한다. 이 레포에는 조회 서비스의 `readOnly` 트랜잭션이 쓰기를 감싸 실제 PostgreSQL 에서 500 이 났던 전례가 있다.

#### 응답 캐시 금지

원본 번호 응답에는 `Cache-Control: no-store` 를 명시한다. `Authorization` 헤더 기반 요청이라 중간 캐시가 저장할 가능성은 낮지만, 브라우저 디스크 캐시에 개인정보가 남는 경로를 열어둘 이유가 없다. 마스킹을 되돌리는 UX 설계(패널 닫으면 사라짐)와도 어긋난다.
...
Swagger `@Operation` 에는 회장 전용이라는 점과 **감사 로그가 남는다는 사실**을 적고, `@ApiResponse` 로 상태 코드의 의미를 명시한다. 특히 **404 는 "그 동아리에 없는 멤버" 를 뜻한다** — 존재하지 않는 memberId 와 다른 동아리의 memberId 를 구분하지 않는다(존재 여부를 숨긴다). 403 은 회장이 아니거나 비-ACTIVE 동아리다. 이 구분을 문서에 남기지 않으면 프론트가 404 를 "삭제된 회원" 으로 오해해 잘못된 안내를 띄우게 된다.
...
| 조회 중 | 마스킹 유지 + 버튼 `disabled` + 진행 표시(`ButtonSpinner`) |
...
조회 중에는 버튼을 `disabled` 로 둔다. 연타하면 같은 조회가 여러 번 나가 감사 로그가 부풀고, 응답 순서가 뒤바뀌면 표시가 요동친다. 진행 표시는 레포 공용 `ButtonSpinner` 를 쓴다(로딩 UI 컨벤션: 텍스트 로딩 금지, 버튼 라벨 유지).
...
- 응답에 `Cache-Control: no-store` 가 실린다.
...
- 누르기 전에는 복사 버튼이 없다.
- 조회 중에는 [번호 보기]가 `disabled` 라 연타로 중복 조회가 나가지 않는다.
