...
컨테이너 `healthcheck`는 `/actuator/health/liveness`를 호출한다. `restart: unless-stopped`는 Docker의
`unhealthy` 상태만으로 컨테이너를 자동 재시작하지 않지만, 상태 신호 자체가 프로세스 생존 의미와 일치하도록
정리한다.
...
배포 워크플로는 Docker의 liveness 기반 `healthy` 값만 기다리지 않는다. 새 컨테이너 내부에서
`/actuator/health/readiness`를 최대 약 100초 동안 재시도한다.
...
- 세 공개 응답 모두 `components`와 `details`를 노출하지 않는다.

### 7.2 DB 장애 인수 테스트

별도 Spring context에서 Hikari DataSource를 명시적으로 종료해 기동 후 DB 연결 장애를 재현한다.
...
DB 마이그레이션과 API 비즈니스 계약 변경은 없다. 문제가 발생하면 이전 백엔드 이미지와 이전 배포 설정으로
함께 롤백한다. 기존 `/actuator/health`를 유지하므로 이전 자동화와 수동 점검 경로도 즉시 사용할 수 있다.
...
- 기존 시설 대관 FE 작업 worktree에는 변경이 발생하지 않는다.
