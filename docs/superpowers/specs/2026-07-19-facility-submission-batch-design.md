...
- 성공 토스트: "제출 목록이 만들어졌어요. 학교 제출 후 '제출 목록' 탭에서 완료 처리해 주세요." (자동 다운로드 없음 유지) → 후보 무효화(제출 필요→제출함 자동 반영). excluded 는 수동 정리하지 않는다 — 재조회 후 화면 기준 프루닝이 담당하며, 제출에서 제외했던 예약은 체크 해제 상태로 남는다(재제출 사고 방지).
