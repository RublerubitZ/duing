...
- 섹션 액션: **"학교 제출하기 (N건)"** — 시설 단위(Batch 단일 시설 도메인 제약 그대로). N = 그 시설의 선택 건수, 0건이면 비활성.
- **Dialog(학교 제출 시작의 마지막 확인)**: 제목 `"{시설명} 예약 N건을 학교에 제출할까요?"`, 본문 "선택한 예약으로 제출 목록을 만들어요. 학교 행정실 제출은 담당자가 진행하고, 제출을 마치면 '제출 목록' 탭에서 완료 처리해 주세요." + memo(선택), 버튼 [취소 / **학교 제출하기**]. Batch 는 내부 개념으로만.
